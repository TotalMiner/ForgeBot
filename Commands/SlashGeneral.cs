...
namespace ForgeBot.Commands
{
    ...
    public class SlashGeneral : ApplicationCommandModule
    {
        ...
        public async Task WikiItem(InteractionContext ctx, [Description("ID of in game item")][Option("item", "Item or Block to review")][RemainingText] string itemid)
        {
            ...
            if (_item == null)
            {
                ExtractedResult<string> autoCorrectedNames;
                if (itemid.Any(char.IsUpper))
                    autoCorrectedNames = Process.ExtractOne(itemid.Replace(" ", ""), GameItem.ItemList, (s) => s, ScorerCache.Get<DefaultRatioScorer>());
                else
                    autoCorrectedNames = Process.ExtractOne(itemid.Replace(" ", ""), Array.ConvertAll(GameItem.ItemList, i => i.ToLower()), (s) => s, ScorerCache.Get<DefaultRatioScorer>());
            ...
            // Reach
            if (_item.StrikeReach > 0 && _item.StrikeReach != 1)
            ...
            var infoButton = new DiscordButtonComponent(ButtonStyle.Secondary, "Info_Button", "Info", true, new DiscordComponentEmoji("📇"));
            var craftButton = new DiscordButtonComponent(ButtonStyle.Primary, "Craft_Button", "Craft [Beta]", true, new DiscordComponentEmoji("⚒️"));
